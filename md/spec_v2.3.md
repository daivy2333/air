...
本区块以有向图的形式描述单元之间，以及单元与外部实体之间的依赖关系。
...
"[外部标识符]" 引用系统库、外部包或标准库中的实体。标识符应尽可能明确。 
"u0->include:[stdio.h]" <br> 
"u1->import:[java.util.List]" PIR 解析器不验证其存在性，由生成工具保证其描述准确性。

"uX#<符号名>" 引用本项目内特定单元的具体符号，提供符号级精度。
"<符号名>" 如函数、变量名。 
"u2->call:u1#main" 目标 
"uX" 必须在 
"<units>" 区块中有定义。符号 
...
动词用于描述依赖的性质，其语义在与不同类型的目标结合时保持一致性。
...
"link" 链接时依赖（如链接脚本被主程序引用）。 
"u0->link:u3"

语义规则与约束

- 唯一性：一个依赖关系 
"uA-><verb>:<target>" 在文件中应是唯一的。
- 内部引用验证：当 
"<target>" 为 
"uX" 或 
"uX#<symbol>" 时，解析器必须验证 
"uX" 已在 
"<units>" 区块中定义。对于符号级依赖，强烈建议生成器确保 
"<symbol>" 在 
"<symbols>" 区块中有定义，以支持高级语义分析。
- 外部引用：外部标识符旨在描述已知的、项目之外的关键依赖。PIR 规范不关心其具体实现位置，工具链也不验证其存在性。
- 非功能性：PIR 依赖关系不保证以下信息，这些通常由构建系统管理：
   - 链接顺序 (Link Order)
