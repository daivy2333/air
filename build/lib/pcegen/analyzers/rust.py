...
class RustAnalyzer(BaseAnalyzer):
    ...
    def extract_implementation(self, symbol_name: str) -> List[str]:
        ...
        if struct_match:
            ...
            if fields:
                ...
                for field in fields:
                    visibility = "pub " if field['visibility'] == 'public' else ""
                    default = f" = {field['default']}" if field['default'] else ""
                    implementation.append(f"  {visibility}{field['name']}: {field['type']}{default},")

            # 提取impl块中的方法
            methods = self.extract_impl_methods(symbol_name)
            if methods:
                ...
                for method in methods:
                    ...
                    trait_str = f" (trait: {method['trait']})" if method['trait'] else ""

                    implementation.append(f"  {modifier_str}fn {method['name']}({method['args']}){return_str}{trait_str}")
    ...
    def extract_behavior(self, symbol_name: str) -> List[str]:
        ...
        if struct_match:
            ...
            if fields:
                ...
                for field in fields:
                    visibility = f"[{field['visibility']}] " if field['visibility'] == 'public' else ""
                    default = f" = {field['default']}" if field['default'] else ""
                    behaviors.append(f"    - {visibility}{field['name']}: {field['type']}{default}")
            ...
            if methods:
                ...
                for method in methods:
                    ...
                    behaviors.append(f"    - {method['name']}{modifier_str}{return_str}{trait_str}")
        else:
            # 函数行为
            in_function = False
            for line in self.lines:
    ...
    def get_implementation(self, source_code: str, func_name: str) -> Optional[str]:
        """提取函数实现"""
        # 简单但有效的正则匹配
        # 匹配 fn function_name(...) -> ... { ... }
        pattern = rf'fn\s+{func_name}\s*\([^)]*\)\s*(?:->[^{{}}]*)?\s*{{[^}}]*}}'
        match = re.search(pattern, source_code, re.DOTALL)
        ...
        for line in lines:
            if f'fn {func_name}' in line and not in_function:
                in_function = True
